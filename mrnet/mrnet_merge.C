...
// A unique signature that differentiates tags of incompatible types from each other
 class  tagGroup2 {
  ...
  tagGroup2(properties::tagType type, const properties* props, MergeInfo::mergeKey key) {
    this->type = type;
    objName = props->name();
/*    if(properties::exists(props->begin(), "callPath")) {
  ...
  bool operator==(const tagGroup2& that) const
...
int merge(vector<MRNetParser*>& parsers,
                   ...
                   structure::MRNetostream* out,
                   mergeType mt, Stream* strm, Network* net, int strm_id, int tag_id,
#ifdef VERBOSE
...
int mergeTags(properties::tagType type, string objName,
              ...
              std::map<std::string, streamRecord*>& outStreamRecords,
              std::vector<std::map<std::string, streamRecord*> >& inStreamRecords,
              int& stackDepth, structure::MRNetostream& out,
              string indent) {
...
/*          dbg << "key=";
          ...
          tag2stream[tagGroup2(props.first, props.second,
                              //(mt==commonMerge? 
...
//      fprintf(stdout, "[MRNetMerge() [#6] PID : %d number of iterators : %d ]\n", getpid(), parsers.size());
    ...
    // If we read text on any parser, emit the text immediately
      ...
      for(map<tagGroup2, StreamTags >::iterator i=tag2stream.begin(); i!=tag2stream.end(); i++) {
    ...
    // If we did not read text on any parser
      ...
      for(map<tagGroup2, StreamTags >::iterator ts=tag2stream.begin(); ts!=tag2stream.end(); ) {
      ...
      // merge() call. At this point we'll read the next tag from them and see if they can be merged.
        ...
        for(map<tagGroup2, StreamTags >::iterator ts=tag2stream.begin(); ts!=tag2stream.end(); variantID++) {
          #ifdef VERBOSE
          ...
          if(ts->first.type == properties::enterTag) {
            ...
            vector<MRNetParser*> groupParsers;
            collectGroupVectorIdx<MRNetParser*>(parsers, ts->second.parserIndexes, groupParsers);
            ...
            // common tag
            map<tagGroup2, StreamTags > groupTag2stream;
            ...
            if(mt == zipper) {
              ...
              structure::MRNetostream* groupStream = new structure::MRNetostream(strm, net, strm_id, tag_id,
                      NULL, txt()<<"Variant "<<subDirCount, subDir, imgDir, tmpDir);
